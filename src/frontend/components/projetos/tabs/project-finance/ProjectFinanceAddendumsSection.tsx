...
import { AlertIcon, ChevronDownIcon, PlusIcon, TrashIcon } from '@/components/ui/icons';
...
export const ProjectFinanceAddendumsSection = ({
  ...
  const [showAddendumsList, setShowAddendumsList] = useState(false);
  const [showNewAddendum, setShowNewAddendum] = useState(false);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-surface rounded-2xl shadow-soft border border-border-color overflow-hidden">
        <button
          type="button"
          onClick={() => setShowNewAddendum((prev) => !prev)}
          className="w-full px-6 py-4 bg-background/30 flex items-center justify-between cursor-pointer hover:bg-background/50 transition-colors"
        >
          ...
          <ChevronDownIcon
            className={`w-5 h-5 text-text-secondary transition-transform duration-200 ${showNewAddendum ? 'rotate-180' : ''}`}
          />
        </button>

        {showNewAddendum && (
          <div className="border-t border-border-color/50 animate-fade-in-up">
            <div className="px-6 py-3">
              {/* Mode toggle */}
              <div className="flex gap-1 bg-background/60 rounded-lg p-0.5 w-fit">
                <button
                  type="button"
      ...
      <div className="bg-surface rounded-2xl shadow-soft border border-border-color overflow-hidden">
        <button
          type="button"
          onClick={() => setShowAddendumsList((prev) => !prev)}
          className="w-full px-6 py-4 bg-background/30 flex items-center justify-between cursor-pointer hover:bg-background/50 transition-colors"
        >
          <div className="flex items-center gap-3">
          ...
          <ChevronDownIcon
            className={`w-5 h-5 text-text-secondary transition-transform duration-200 ${showAddendumsList ? 'rotate-180' : ''}`}
          />
        </button>

        {showAddendumsList && (
          <div className="p-5 border-t border-border-color/50 animate-fade-in-up">
            {addendums.length === 0 ? (
            ...
            {auditTrail.length > 0 && (
              <button
                ...
                className="flex items-center gap-1 text-[11px] font-semibold text-text-secondary hover:text-warning transition-colors mt-3"
              >
