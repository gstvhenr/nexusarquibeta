...
const SESSION_KEY = 'nexus-drive-api-toast-shown';

type DriveApiToastProps = {
  visible: boolean;
  onDismiss: () => void;
};

function DriveApiToast({ visible, onDismiss }: DriveApiToastProps): JSX.Element | null {
      ...
      <div className="flex items-center gap-4 rounded-xl bg-surface-card px-5 py-3 shadow-lifted ring-1 ring-border-color/30">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-text-primary">
            Conexão estabelecida com o Google Drive
          </span>
        </div>
        <button
          type="button"
